...
class InventoryManagementEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.agents = [f"stage_{i}" for i in range(num_stages)]
    ...
    def reset(self, *, seed=None, options=None) -> tuple[dict, dict]:
        ...
        self.update_state()
        observations = {
            f"stage_{i}": self.state_dict[f"stage_{i}"]  # adjust as needed based on your 
            for i in range(self.num_stages)
        }
        # If self.state_dict is already a dict with the right keys, you can simply do:
